...
if use_filter == 1 or use_filter == 2:
    ...
    n, wn = signal.cheb1ord(wp, ws, rp, abs(rs))
    b, a = signal.cheby1(n, rp, wn, filter_name[use_filter - 1])
    ...
    n, wn = signal.cheb1ord([wp1, wp2], [ws1, ws2], rp, abs(rs))
    b, a = signal.cheby1(n, rp, wn, filter_name[use_filter - 1])
else:
...
ax[0, 0].set_title(f'{filter_name[use_filter - 1].capitalize()} Chebyshev Type I Filter Response')
ax[0, 0].grid()
...
ax[0, 1].set_title(f'{filter_name[use_filter - 1].capitalize()} Chebyshev Type I Filter Response')
ax[0, 1].grid()
