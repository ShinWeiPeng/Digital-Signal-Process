...
if use_filter == 1 or use_filter == 2:
    ...
    b, a = signal.ellip(n, rp, abs(rs), wn, filter_name[use_filter - 1])
    ...
    n, wn = signal.ellipord([wp1, wp2], [ws1, ws2], rp, abs(rs))
    b, a = signal.ellip(n, rp, abs(rs), wn, filter_name[use_filter - 1])
else:
...
ax[2, 1].plot(w, gd, label='Group Delay (s)')
ax[2, 1].set_xlabel('Frequency (Hz)')
ax[2, 1].set_ylabel('delay (s)')
ax[2, 1].grid()
ax[2, 1].legend()
